...
Each of these components has it's set of DOF's which can be described mathematically. The established convention for describing this kind of linked systems is the \textit{Denavit-Hartenberg} convention (D-H)\cite{Denavit.1955}. It was actually introduced for the order to standardize the coordinate frames for spatial linkages in the field of robotics, but has since thean be applied in other fields as well.\cite{Spong.2008}\todo{Denavit Hartenberg formmulation for coordinate System definition and transformation}  \\
...
\todo{ add other missing methods, maybe only as further are}
...
\todo{review of algorithm capacilities}